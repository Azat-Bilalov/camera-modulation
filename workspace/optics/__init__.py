...
from workspace.models import OpticsConfig, SensorExposure

from workspace.scene_source import SceneSignal, SpectralAxis
...
class OpticsInput:
    ...
    def axis(self) -> SpectralAxis:
        return SpectralAxis(wave=self.wavelengths_nm, start = self.wavelengths_nm[0], stop=self.wavelengths_nm[-1], bands_count=len(self.wavelengths_nm))
...
def read_optics_input(scene: list[float], axis: SpectralAxis | None = None) -> OpticsInput:
    """Read SceneSignal-like input from workspace/scene_source without importing that package."""

    wavelengths = axis.wave
    signal = scene
    optics_input = OpticsInput(wavelengths_nm=wavelengths, spectra=signal)
