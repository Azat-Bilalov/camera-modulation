...
TODO(sensor_adc): диафрагма ахроматична (геометрическая оптика, без призмы/
дисперсии), поэтому разбиение спектра на R/G/B-каналы сюда не относится —
его нужно делать в слое сенсора (по спектральной чувствительности/CFA),
там же применять `OpticsConfig.transmission` и `rgb_ranges_nm` (см. также
TODO в `sensor_adc/sensor_pipeline.py`). Здесь на выход отдаётся полный
...
def project_scene_through_pinhole(
    ...
    разбиение по длинам волн переносится на слой сенсора (см. TODO в начале
    ...
    return sensor_exposure  # 3D: H×W×bands (TODO(sensor_adc): свернуть в H×W×3)
